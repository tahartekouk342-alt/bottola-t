...
export default function Following() {
  ...
  return (
    <div className="container mx-auto px-4 py-8" dir="rtl">
      <div className="mb-8">
        <h1 className="font-display text-3xl md:text-4xl font-bold mb-2">المتابعات</h1>
          ...
          {loadingFollowing ? (
            ...
            <div className="grid gap-5 md:grid-cols-2 lg:grid-cols-3">
              {following.map((profile) => (
                <Card key={profile.id} className="card-interactive cursor-pointer rounded-3xl" onClick={() => navigate(`/viewer/organizer/${profile.user_id}`)}>
      ...
      <CardHeader className="pb-4">
        <div className="flex items-start gap-4">
          ...
          <div className="flex-1 min-w-0">
            ...
            <p className="text-sm text-muted-foreground line-clamp-2 mt-1">{organizer.bio || 'منظم بطولات رياضية'}</p>
          </div>
        </div>
      </CardHeader>
      <CardContent className="space-y-5">
        <div className="flex items-center gap-6 text-sm">
          <div className="flex items-center gap-2"><Trophy className="w-4 h-4 text-primary" /><span className="font-bold">{organizer.tournament_count || 0}</span><span className="text-muted-foreground">بطولة</span></div>
          <div className="flex items-center gap-2"><Users className="w-4 h-4 text-primary" /><span className="font-bold">{organizer.follower_count || 0}</span><span className="text-muted-foreground">متابع</span></div>
        </div>
        <div className="flex gap-3 pt-2">
          <Button variant="outline" className="flex-1 rounded-2xl border-white/10 hover:bg-secondary/50" onClick={(e) => { e.stopPropagation(); onViewTournaments(); }}>عرض البطولات</Button>
          {organizer.is_following ? (
            <Button variant="ghost" size="icon" onClick={(e) => { e.stopPropagation(); onUnfollow(); }} className="text-destructive hover:text-destructive hover:bg-destructive/10 rounded-2xl"><UserMinus className="w-5 h-5" /></Button>
          ) : (
            <Button size="icon" onClick={(e) => { e.stopPropagation(); onFollow(); }} className="btn-primary rounded-2xl"><UserPlus className="w-5 h-5" /></Button>
          )}
        </div>
      </CardContent>
    </Card>
