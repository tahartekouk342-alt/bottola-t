import { Bell, CheckCheck, Trophy, Loader2 } from 'lucide-react';
      ...
      {notifications.length === 0 ? (
        ...
        <div className="space-y-3">
          {notifications.map((notification) => (
            <Card
              key={notification.id}
              className={cn("transition-all duration-200 cursor-pointer", !notification.is_read && "border-primary/30 bg-primary/5")}
              onClick={() => {
            ...
            >
              <CardContent className="p-4 flex items-start gap-4">
                <div className={cn("w-10 h-10 rounded-xl flex items-center justify-center flex-shrink-0", !notification.is_read ? "gradient-primary" : "bg-muted")}>
